...
> ⚠️ **本节的「固定延迟」裁决已于实现阶段被推翻(2026-08-12,T036)**,原文以下内容**保留原样以备追溯,但不得作为实现依据**。
> 推翻理由:`Thread.sleep` 占用的是 Tomcat 工作线程,而 `POST:/auth/tokens` 已加白(未认证可达)→ 成本不对称在攻击者一侧,200 线程下约 1000 req/s 即可拖垮全站(无延迟需 ~200,000 req/s);且 FR-015 清空 `refresh_token:*` 后会自我触发 —— **与本节否决 503 的理由同源,当时只否了 503 却漏了延迟**。
> 现行裁决见 `plan.md` §3.3(v4)与 `docs/reviews/2026-08-12-007-security-reviewer.md` HIGH-1。
