...
public class BlueprintsApplicationService {
    ...
    public PageResult<BlueprintsVO> queryBlueprintsByPage(BlueprintsQuery query) {
        BlueprintsPageCriteria criteria = toCriteria(query);
     ...
     */
    private BlueprintsPageCriteria toCriteria(BlueprintsQuery query) {
        return BlueprintsPageCriteria.builder()
                .ownerId(requireOwnerId(query.getOwnerId()))
    ...
    private String requireOwnerId(String ownerId) {
        ...
        if (!DIGITS.matcher(trimmed).matches()) {
            throw new EveHelperException("所有者ID必须为数字：" + ownerId);
        }
        return trimmed;
    }
