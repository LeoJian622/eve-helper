...
/**
 * AssetsPoConverter 双向映射测试(MapStruct 生成实现)。
 *
 ...
 */
@DisplayName("AssetsPoConverter 双向映射")
class AssetsPoConverterTest {

    private final AssetsPoConverter converter = new AssetsPoConverterImpl();

    @Test
    @DisplayName("domain2Po + po2Domain 往返后 userId 保持")
    void roundTrip_preservesUserId() {
        Assets domain = new Assets();
        domain.setItemId(1017113140391L);
        domain.setOwnerId(2112832425L);
        domain.setUserId(42L);

        AssetsPO po = converter.domain2Po(domain);
        assertThat(po.getUserId()).isEqualTo(42L);

        Assets back = converter.po2Domain(po);
